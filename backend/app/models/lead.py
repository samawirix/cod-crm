...
class LeadSource(str, PyEnum):
    ...
    INSTAGRAM = "INSTAGRAM"
    WHATSAPP = "WHATSAPP"
    TIKTOK = "TikTok"      # Added for existing database entries
    REFERRAL = "REFERRAL"
    OTHER = "OTHER"
...
class LeadStatus(str, PyEnum):
    ...
    CONTACTED = "CONTACTED"
    QUALIFIED = "QUALIFIED"
    CONFIRMED = "CONFIRMED"  # Order confirmed from this lead
    PROPOSAL = "PROPOSAL"
...
class Lead(Base):
    ...
    call_notes = relationship("CallNote", back_populates="lead", cascade="all, delete-orphan")
    ...
    def validate_email(self, key: str, email: str) -> str:
        ...
        Returns:
            The validated email address or empty string
            
        Raises:
            ValueError: If email format is invalid
        """
        # Email is optional - return empty string if not provided (DB has NOT NULL)
        if not email:
            return ""
    ...
    def validate_name(self, key: str, value: str) -> str:
        ...
        Raises:
            ValueError: If first_name is empty or invalid
        """
        # Only first_name is required, last_name is optional
        if key == 'first_name':
            if not value or not value.strip():
                raise ValueError(f"{key.replace('_', ' ').title()} is required")
            return value.strip()
        else:
            # last_name is optional, just strip if provided
            if value:
                return value.strip()
            return value
